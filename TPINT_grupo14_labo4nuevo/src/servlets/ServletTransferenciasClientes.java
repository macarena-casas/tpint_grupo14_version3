	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (request.getParameter("btnTransferir") != null) {
			...
			if (!transferirMismaCuenta(request)) {
				...
				int insert = tNegocio.insert(cbuOrigen, cbuDestino, detalle, importe);

				if (insert == 0) {
					session.setAttribute("respuesta", "Transferencia realizada!");

				} else {
					if (insert == 45000) {
						throw new Exception("Error: " + String.valueOf(insert)
								+ ", Saldo insuficiente para realizar la transferencia.");
					} else {
						throw new Exception(
								"Error: " + String.valueOf(insert) + ", no fue posible realizar la transferencia.");
					}
		...
		}else if(request.getParameter("btnTransferir2")!=null){
			try {
